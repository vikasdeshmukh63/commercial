...
import {
  ...
  deleteDRSiteItem,
  reset,
} from '../store/slices/drsiteSlice';
import { getSummaryByOppId } from '../store/slices/summarySlice';

function DRSite() {
  const { oppId } = useParams();
  const dispatch = useDispatch();
  const { items, isLoading, error } = useSelector((state) => state.drsite);
  ...
  const handleChange = (e) => {
    ...
    } else if (name.startsWith('rpoRto.')) {
      const rpoRtoField = name.split('.')[1];
      setFormData({
        ...formData,
        rpoRto: {
          ...formData.rpoRto,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        await dispatch(updateDRSiteItem({ 
          oppId, 
          id: selectedItem._id, 
          itemData 
        })).unwrap();
      } else {
        await dispatch(createDRSiteItem({ oppId, itemData })).unwrap();
      }
      handleClose();
      // Refresh the list
      dispatch(getDRSiteItems(oppId));
    } catch (error) {
      console.error('Error saving DR Site item:', error);
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this DR Site configuration?')) {
      try {
        await dispatch(deleteDRSiteItem({ oppId, id })).unwrap();
        dispatch(getDRSiteItems(oppId));
      } catch (error) {
        console.error('Error deleting DR Site item:', error);
      }
    }
  };
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h4">Disaster Recovery Site</Typography>
          {currentSummary && (
        ...
        <CardContent>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={4}>
              ...
              <Typography variant="h4">
                {formatCurrency(totals.totalMRC)}
              </Typography>
            </Grid>
            <Grid item xs={12} sm={4}>
              <Box display="flex" alignItems="center" mb={1}>
              ...
              <Typography variant="h4">
                {formatCurrency(totals.totalOTC)}
              </Typography>
            </Grid>
          </Grid>
        </CardContent>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>DR Environment</TableCell>
                  <TableCell>DR Location</TableCell>
              ...
              <TableBody>
                {items?.map((item) => (
                  <TableRow key={item._id} hover>
                    ...
                    <TableCell>
                      <Typography variant="body2">
                        <strong>RPO:</strong> {item.rpoRto?.rpo || 'N/A'}
                      </Typography>
                      <Typography variant="body2">
                        <strong>RTO:</strong> {item.rpoRto?.rto || 'N/A'}
                      </Typography>
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2">
                ...
                {(!items || items.length === 0) && (
                  <TableRow>
                    <TableCell colSpan={canEdit ? 11 : 10} align="center">
                      <Typography color="textSecondary">
                        No DR Site configurations found. Add DR servers to get started.
                      </Typography>
                    </TableCell>
                  </TableRow>
                )}
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
        <DialogTitle>
          {editMode ? 'Edit DR Site Configuration' : 'Add DR Site Configuration'}
        </DialogTitle>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  margin="dense"
                  name="serverName"
                  label="DR Server Name"
                  fullWidth
                  variant="outlined"
                  value={formData.serverName}
                  onChange={handleChange}
                  placeholder="e.g., DR-WEB-01"
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth margin="dense">
                  <InputLabel>DR Environment</InputLabel>
                  <Select
                    name="environment"
                    ...
                    onChange={handleChange}
                  >
                    <MenuItem value="Production DR">Production DR</MenuItem>
                    <MenuItem value="Staging DR">Staging DR</MenuItem>
                    <MenuItem value="Development DR">Development DR</MenuItem>
                    <MenuItem value="Testing DR">Testing DR</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
              <Grid item xs={12}>
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  value={formData.drLocation}
                  onChange={handleChange}
                  required
                  placeholder="e.g., Mumbai DR Center, Bangalore Backup Site"
                />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  ...
                  label="DR Hosting Components"
                  fullWidth
                  variant="outlined"
                  value={formData.hostingComponents}
                  onChange={handleChange}
                  required
                  placeholder="e.g., Backup Web Server, Database Replica, File Storage"
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  value={formData.rpoRto.rpo}
                  onChange={handleChange}
                  placeholder="e.g., 1 hour, 4 hours, 24 hours"
                  helperText="Maximum acceptable data loss"
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  value={formData.rpoRto.rto}
                  onChange={handleChange}
                  placeholder="e.g., 30 minutes, 2 hours, 8 hours"
                  helperText="Maximum acceptable downtime"
                />
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={handleChange}
                  placeholder="e.g., PostgreSQL 13 Replica"
                />
              </Grid>
